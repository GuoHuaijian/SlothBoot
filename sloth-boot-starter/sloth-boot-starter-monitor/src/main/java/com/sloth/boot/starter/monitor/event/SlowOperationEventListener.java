...
public class SlowOperationEventListener {
    ...
    public void onSlowOperation(SlowOperationEvent event) {
        log.warn("[Monitor] 慢操作: type={}, cost={}ms, threshold={}ms, detail={}",
            event.operationType(), event.costTimeMs(), event.thresholdMs(), event.detail());

        Timer.builder("sloth.slow.operation")
            .tag("type", event.operationType())
            .register(meterRegistry)
            .record(event.costTimeMs(), TimeUnit.MILLISECONDS);

        if (alarmService != null) {
            AlarmMessage alarmMessage = new AlarmMessage();
            alarmMessage.setTitle("慢操作告警");
            StringBuilder content = new StringBuilder();
            content.append("类型: ").append(event.operationType());
            content.append("\n详情: ").append(truncate(event.detail(), 500));
            content.append("\n耗时: ").append(event.costTimeMs()).append("ms");
            content.append("\n阈值: ").append(event.thresholdMs()).append("ms");
            if (event.context() != null) {
                content.append("\n上下文: ").append(event.context());
            }
